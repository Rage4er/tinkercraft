...
## 💎 ФИКСЫ ЭКОНОМИКИ UI — 2026-08-28

| Метрика | Значение |
|---------|----------|
| Проверено по ECONOMY.md v2.0 | 21 требование §6.1–6.7 |
| Выполнено до | ~14 (67%) |
| **Выполнено после** | **21 (100%)** |
| **Статус** | **✅ Готово** |

### Исправлено

- ✅ **§6.1** — Счётчик `+50 · N/3` в ХУДе, состояние "до 00:00" для бонуса
- ✅ **§6.1** — Состояние "лимит дня" / "ждём 4:32" для рекламы
- ✅ **§6.3** — Кнопка скрытия баннера **на самом баннере** (место 1)
- ✅ **§6.4** — Бейджи на кнопках: 💰/📺 слева-внизу, ⏱/👑 слева-вверху
- ✅ **§6.5** — Разбивка кэшбэка в модалке экспорта
- ✅ **§6.6** — i18n ключи дополнены (export.cashbackBreakdown, adLabel)
- ✅ **§6.7** — Онбординг 3 шага с флагом в облако
- ✅ **Bug** — EconomyShop.tsx: хук в цикле → `getState()`

### Новые файлы
- `components/Badge.tsx` — бейджи на кнопках
- `components/EconomyBanner.tsx` — рекламный баннер
- `components/EconomyOnboarding.tsx` — онбординг экономики
...
## 💾 ФИКСЫ ХРАНЕНИЯ ДАННЫХ (syncToCloud) — 2026-08-30
...
### Проблема
3 метода экономики не вызывали `syncToCloud()`, а сам `syncToCloud()` не сохранял daily-счётчики. Это означало потерю прогресса при обновлении страницы для неавторизованных пользователей.

### Исправлено
- ✅ `earnActionToken()` → добавлен `syncToCloud()`
- ✅ `calculateAndClaimCashback()` → добавлен `syncToCloud()`
- ✅ `completeEventQuest()` → добавлен `syncToCloud()`
- ✅ `syncToCloud()` → добавлены поля: `todayAdsWatched`, `todayActions`, `todayCashbacks`, `questTriggers`
- ✅ `loadFromCloud()` → добавлена загрузка daily-счётчиков
- ✅ `partialize()` → добавлены daily-счётчики в localStorage кэш

### Новые файлы
- `STORAGE_POLICY.md` — документация политики хранения данных
...
### 🟡 Y3.14 — `evaluateQuests()` определён, но НИКОГДА не вызывается — CRITICAL
...
`evaluateQuests()` (economy-store.ts:565) — функция, которая сканирует сцену и обновляет прогресс квестов по состоянию проекта:
- `count_cubes` — количество кубов
- `count_objects` — количество объектов
- `count_unique_shapes` — уникальные типы примитивов
- `count_colored` — объекты с цветом
- `count_mirrored` — зеркала
- `count_csg` — CSG-узлы
- `csg_complex` — CSG с ≥3 детьми
- `count_text3d` — 3D-тексты

**Комментарий в App.tsx:238 подтверждает:** `// evaluateQuests теперь только обновляет прогресс (без начисления токенов)`.
Но функция **нигде не вызывается** — квесты по состоянию проекта (12 из 15 в пуле) никогда не обновляются.

**Влияние:** Игроки получают квесты "Построй 5 кубов" / "Используй 4 разных фигуры" и т.д., но прогресс никогда не считается. Квесты невозможно выполнить.

**Где должен быть вызов:** После каждого изменения сцены в `document-store.ts` — после любого `set({ operations, objects })`.

**Файлы:**
- `src/store/economy-store.ts` — функция определена (строка 565), но не вызывается
- `src/store/document-store.ts` — нужно добавить вызов после мутаций сцены
- `src/App.tsx` — комментарий на строке 238 подтверждает намерение

#### Решение

Добавить вызов `evaluateQuests()` в 14 мутающих методов document-store.ts после `set({ operations, objects })`:
- `csgBoolean` → после set (строка 584)
- `addShape` → после set (строка 251)
- `applyFillet` → после set (строка 340)
- `importStl` → после set (строка 309)
- `pasteClipboard` → после set (строка 434)
- `deleteSelected` → после set (строка 469)
- `mirrorSelected` → после set (строка 734)
- `csgAlign` → после set (строка 751)
- `resizeObject` → после set (строка 1098)
- `undo` → после set (строка 919)
- `redo` → после set (строка 1211)
- `loadFromDoodle` → после set (строка 995)
- `loadFromProject` → после set (строка 1294)
- `createTextMesh` → после set

---

### 🟡 Y3.15 — EconomyShop.tsx существует, но не используется — MEDIUM
...
Компонент `EconomyShop.tsx` существует и реализован (аренда + подписки + кнопка скрытия баннера), но **не импортируется и не рендерится** в `App.tsx`. Вместо этого функциональность дублирована в `EconomyPanel` внутри `PropertiesPanel.tsx`.

**ECONOMY.md §6.3:** требует кнопку скрытия баннера в **трёх местах**: баннер, панель (PropertiesPanel), магазин (EconomyShop).

Реализовано только 2 из 3.

**Файлы:**
- `src/components/EconomyShop.tsx` — существует, но не используется
- `src/App.tsx` — не импортирует EconomyShop

---

## 📋 Активные проблемы

> **Статус: 2 новые проблемы обнаружены при аудите соответствия ECONOMY.md v2.0**
...
## Y3.16 — Бейджи на кнопках экономики (§6.4 ECONOMY.md v2.0)

**Статус:** ✅ ИСПРАВЛЕНО (03.09.2026)

**Проблема:** На платных кнопках экономики не отображались бейджи стоимости (§6.4 ECONOMY.md).

**Карта бейджей по ECONOMY.md §6.4:**

| Кнопка | Бейдж | Место | Статус |
|--------|-------|-------|--------|
| Экспорт (50/1📺) | 💰50 / 📺1 | Toolbar.tsx | ✅ было |
| Импорт (100/2📺) | 💰100 / 📺2 | Toolbar.tsx | ✅ было |
| Текст (75) | 💰75 | LeftPanel.tsx | ✅ было |
| **Палитра (75)** | **💰75** | **PropertiesPanel.tsx** | **✅ исправлено** |
| **Баннер off (50/1📺)** | **💰50 / 📺1** | **EconomyBanner.tsx** | **✅ исправлено** |
| **Аренда (текст/палитра/баннер)** | **💰75 / 💰75 / 💰50** | **PropertiesPanel.tsx (EconomyPanel)** | **✅ исправлено** |
| **Подписки (700/2000)** | **💰700 / 💰2000** | **PropertiesPanel.tsx, EconomyShop.tsx** | **✅ исправлено** |
| **Баннер off** | **💰50 / 📺1** | **EconomyShop.tsx** | **✅ исправлено** |

**Реализация:**
- Добавлены inline-бейджи с `position: absolute` на все платные кнопки
- 💰 слева-внизу (жёлтый #fbbf24) — цена в токенах
- 📺 справа-внизу (фиолетовый #8b5cf6) — количество рекламы
- `pointer-events: none` — бейджи не перехватывают клики
- Разный размер шрифта для компактных кнопок (8px) и обычных (10-11px)

**Файлы:**
- `components/PropertiesPanel.tsx` — бейджи на палитру, аренду, подписки, баннер off
- `components/EconomyBanner.tsx` — бейджи на кнопки баннера off
- `components/EconomyShop.tsx` — бейджи на аренду, подписки, баннер off
