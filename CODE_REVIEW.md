...
| **🔍 Y.3 Экономика — аудит соответствия ECONOMY.md** | **🔄 2026-08-27 — 7/13 исправлено (все критические), 6 не исправлено (2 HIGH, 3 LOW, 1 LOW)** |
...
| Y3.8 | 🟡 HIGH | Нет UI для покупки аренды | ❌ НЕ ИСПРАВЛЕНО |
| Y3.9 | 🟡 HIGH | Нет UI для покупки подписок | ❌ НЕ ИСПРАВЛЕНО |
| Y3.10 | 🟡 HIGH | Кнопка «Бесплатный экспорт» противоречит ECONOMY.md | ✅ ИСПРАВЛЕНО |
| Y3.11 | 🟢 LOW | Нет таймера кулдауна рекламы в UI | ❌ НЕ ИСПРАВЛЕНО |
| Y3.12 | 🟢 LOW | Fallback категорий квестов нарушает правило | ❌ НЕ ИСПРАВЛЕНО |
| Y3.13 | 🟢 LOW | `todayQuestsCompleted` не используется | ❌ НЕ ИСПРАВЛЕНО |
...
ECONOMY.md предусматривает 3 вида аренды (24ч):

| Аренда | Цена | UI | Статус |
|--------|------|-----|--------|
| 3D-текст | 75 токенов | ❌ Нет проверки `hasRental('text3d')` | 🔴 Нет UI |
| Расширенная палитра | 75 токенов | ❌ Нет проверки `hasRental('extendedPalette')` | 🔴 Нет UI |
| Отключение баннера | 50 токенов | ❌ Нет кнопки покупки | 🔴 Нет UI |

Функции `buyRental()` и `hasRental()` реализованы в store, но **нет ни одного UI-компонента** для их использования. TextModal и ColorPalette работают без проверок аренды.
...
ECONOMY.md предусматривает 2 подписки:

| Подписка | Цена | UI | Статус |
|----------|------|-----|--------|
| Недельная | 700 токенов | ❌ Нет кнопок | 🔴 Нет UI |
| Месячная | 2000 токенов | ❌ Нет кнопок | 🔴 Нет UI |

Функция `buySubscription()` реализована, но **нет ни одного UI-экрана** для подписок.
...
В `economy-store.ts` (строка 289) кулдаун 5 минут реализован:

```ts
if (!isCooldownPassed(state.lastAdTimestamp, AD_COOLDOWN_MS)) return false
```

Но в `EconomyHUD.tsx` нет отображения обратного отсчёта до следующей рекламы. Пользователь видит кнопку «Смотреть рекламу», но не видит, когда она станет доступна.
...
В `generateDailyQuestsV2` (economy-store.ts, строки 183–186):

```ts
const chosen = available.length > 0
  ? available[Math.floor(Math.random() * available.length)]
  : pool[Math.floor(Math.random() * pool.length)]  // fallback — без проверки категории
```

Если для выбранной сложности все квесты из уже использованных категорий, берётся случайный без проверки. Это может привести к двум квестам из одной категории.
...
В `evaluateQuests` (economy-store.ts, строка 516) проверяется `!quests.find(q => q.trigger === quest.trigger && q.completed)`, но массив `todayQuestsCompleted` нигде не используется для проверки повторного начисления токенов.
