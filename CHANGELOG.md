...
- **7.5.5 — Layout тулбара (CAD-style)**: Переработана структура тулбара — группы кнопок не переносятся, кнопки внутри групп переносятся на новые строки. Разделители (`toolbar-separator`) на всю высоту контейнера. CSS: `toolbar { flex-wrap: nowrap }`, `toolbar-group { flex-wrap: wrap }`, `separator { align-self: stretch }`. Добавлен горизонтальный скролл при недостатке ширины.
