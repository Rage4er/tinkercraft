...
- **7.5.5 — Layout тулбара (CAD-style)**: Переработана структура тулбара — группы кнопок могут переноситься, кнопки внутри групп переносятся на новые строки. Разделители (`toolbar-separator`) на всю высоту контейнера. CSS: `.toolbar { flex-wrap: wrap }`, `.toolbar-group { flex-wrap: wrap }`, `.toolbar-group .btn { flex-shrink: 1 }`, `separator { align-self: stretch }`.
