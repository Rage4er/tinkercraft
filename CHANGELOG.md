...
- **7.5.5 — Layout тулбара (CAD-style)**: Переработана структура тулбара — группы кнопок всегда на одной строке, кнопки внутри равномерно заполняют пространство. Если toolbar не влазит — горизонтальный скролл. CSS: `.toolbar { flex-wrap: nowrap; overflow-x: auto }`, `.toolbar-group { flex: 1 1 0; display: grid; grid-template-columns: repeat(auto-fit, minmax(34px, 1fr)) }` — группы равномерно делят пространство, кнопки 34x34px центрированы в ячейках grid. `separator { align-self: stretch }`.
