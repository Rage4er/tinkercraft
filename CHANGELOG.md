...
- **MIRROR-6 (CRITICAL)**: Исправлена математика зеркального отражения во ВСЕХ местах. Раньше зеркалилась ось **перпендикулярная** плоскости (неверно). Теперь: ось **перпендикулярная** зеркальной плоскости НЕ меняется, оси **В плоскости** меняют знак. Исправлено в: `history-tree.ts` (mirrorNodeRecursive), `rebuildOps.ts` (applyMirrorToTransform), `worker-handlers.ts` (handleMirrorObject). Пример YZ mirror: `rotX:10, rotY:10, rotZ:10, scaleX:1, scaleY:1, scaleZ:0.5` → `rotX:10, rotY:-10, rotZ:-10, scaleX:1, scaleY:-1, scaleZ:-0.5` (rotX/scaleX НЕ меняются, rotY/rotZ/scaleY/scaleZ инвертируются)
