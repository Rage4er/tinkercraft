...
- **MIRROR-CSG-POSITION-MATH (CRITICAL)**: CSG/import результаты теперь используют правильную математику mirror для позиции: перпендикулярная ось negated, оси В плоскости unchanged. Ранее `2 * mc - pos` зеркалил по ВСЕМ осям (YZ: x:-40, y:40, z:-40 вместо x:-40, y:-40, z:40). Исправление: YZ → negated X, XZ → negated Y, XY → negated Z. Исправлено: `document-store.ts` (mirrorSelected)
