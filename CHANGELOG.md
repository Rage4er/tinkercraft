...
- **7.5.5 — Layout тулбара (CAD-style)**: Переработана структура тулбара — группы кнопок всегда на одной строке (без переноса), кнопки внутри групп переносятся на новые строки. Если toolbar не влазит целиком — горизонтальный скролл. CSS: `.toolbar { flex-wrap: nowrap; overflow-x: auto }`, `.toolbar-group { display: grid; grid-template-columns: repeat(auto-fill, 34px) }` — grid auto-fill автоматически переносит кнопки, без flex-shrink трюков. `separator { align-self: stretch }`.
