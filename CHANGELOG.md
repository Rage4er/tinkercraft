...
- **7.5.5 — Layout тулбара (CAD-style)**: Переработана структура тулбара — группы кнопок всегда на одной строке (без переноса), кнопки внутри групп переносятся на новые строки. Если toolbar не влазит целиком — горизонтальный скролл. CSS: `.toolbar { flex-wrap: nowrap; overflow-x: auto }`, `.toolbar-group { flex-wrap: wrap; flex-shrink: 0 }`, `.toolbar-group .btn { flex-shrink: 1 }`, `separator { align-self: stretch }`. Убран глобальный `flex-shrink: 0` у `.btn` чтобы кнопки могли сжиматься внутри групп.
