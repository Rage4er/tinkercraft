...
- **7.5.5 — Алгоритм распределения кнопок по строкам**: Реализован `toolbar-layout.ts` с итеративным алгоритмом сужения/расширения. Алгоритм определяет `firstRowCount` для каждой группы — сколько кнопок показать в первой строке. Остальные кнопки переносятся на 2-ю и 3-ю строки. Гистерезис 1W предотвращает дребезг при расширении. MAX_ROWS=3 — лимит строк. Интегрирован в Toolbar через `useToolbarLayout` hook. Компоненты `MirrorButtons`, `AlignButtons`, `CsgButtons` модифицированы для приема `maxFirstRow` prop.
