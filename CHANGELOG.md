...
### Fixed
- **UB-0a (экспорт STL — «разлёт» фигур):** `exportToStl` применял `obj.transform` к вершинам, у которых позиция спавна УЖЕ запечена воркером в геометрию (`handleBuildShape` → `m.transform([T])`) — при экспорте нескольких фигур позиция применялась дважды, объекты разлетались на 2× координат. Теперь трансформ применяется к bbox-центрированной геометрии — ровно как во вьюпорте (`centerGeometry` + transform на pivot): `R·S·(v − center) + pos`. CSG-результаты (уже центрированные) не затрагиваются. Портировано из `yandex-games` (`io/stl-export.ts`, `io/stl-export.test.ts`)
- **UB-0b (импортированная геометрия терялась при сохранении/загрузке):** `handleRebuildScene` запоминал для `import_mesh` только transform — вершины не попадали в manifold-кэш, объект отсутствовал в `results`, и после загрузки проекта/`.doodle` импорт исчезал. Теперь baked-геометрия восстанавливается из `op.vertices/indices` (защита по размерам, non-manifold → `null`); `move` над baked-мешом сдвигает кэш на дельту (раньше — no-op / куб-заглушка 20³); фолбэк в `rebuildFromHistory` возвращает сырой меш из операции, если воркер его не вернул (`csg/worker-handlers.ts`, `store/rebuild.ts`)
- **UB-0d (.doodle с импортами сохранялся, но не открывался):** лимит `model.json` поднят 5→64 МБ (защита от ZIP-бомбы 50 МБ на архив осталась), TypedArray сериализуются обычными массивами через `typedArrayReplacer` (раньше `JSON.stringify(Float32Array)` давал объект `{"0":…}` — ~1.4× размер и потеря `length`) (`io/doodle-io.ts`)
- **E1 (молчаливая порча большого STL):** `MAX_TRIANGLES` урезал размер буфера и count в заголовке, но цикл записи лимита не проверял → выход за границу `DataView` (`RangeError`) либо файл с недостоверным числом треугольников. Теперь сверх лимита бросается `StlTooLargeError` с локализованным сообщением (ключ `errors.stlTooManyTris` раньше был мёртвым), лимит вынесен в параметр `exportToStl(objects, maxTriangles)`
- **E1b (ошибка экспорта была невидима):** `exportStl` вызывал `downloadStl()` без `try/catch` прямо из обработчика клика — любое падение сериализации (невалидная геометрия, лимит треугольников) проходило молча. Сериализация (`exportToStl`) отделена от скачивания (`downloadStlBlob`), ошибка показывается через `notify` (`store/document-store.ts`, `io/stl-export.ts`, i18n ru/en: `errors.stlExportFailed`)
- **E2 (STL-заголовок и кириллица):** заголовок писался побайтно через `charCodeAt` — в ru-локали («Творческая студия — экспорт STL») значения > 255 обрезались по младшему байту. Теперь UTF-8 через `TextEncoder` с обрезкой по 80 байт (`io/stl-export.ts`)

### Added
- **Тесты на цикл «экспорт/сохранение → загрузка» (T1):** регрессия UB-0a (запечённая позиция не применяется дважды), лимит треугольников (E1), UTF-8-заголовок (E2); round-trip `import_mesh` через `serializeDoodle → parseDoodle` (массив вместо `{"0":…}`, файл > 5 МБ открывается); `rebuild-baked.test.ts` — фолбэк baked-геометрии при пустом ответе воркера (4 теста, воркер моканут, WASM не нужен) (`io/stl-export.test.ts`, `io/doodle-io.test.ts`, `store/rebuild-baked.test.ts`)
