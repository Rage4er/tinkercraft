...
function buildPrimitive(shapeType: string, params: Record<string, number>): M {
  ...
  switch (shapeType) {
    case 'cube':
      // width=X, height=Y(up), depth=Z — правильный порядок осей
      return Manifold.cube([params.width ?? 20, params.height ?? 20, params.depth ?? 20], true)
    case 'sphere':
      return Manifold.sphere(params.radius ?? 12, params.segments ?? 32)
    case 'cylinder':
      return Manifold.cylinder(params.height ?? 30, params.radius ?? 10, params.radius ?? 10, params.segments ?? 32, true)
    case 'cone':
      return Manifold.cylinder(params.height ?? 30, params.radius ?? 10, 0, params.segments ?? 32, true)
    default:
...
function buildPrimitiveWithFillet(shapeType: string, params: Record<string, number>, r: number): M {
  if (r > 0 && shapeType === 'cube') {
    // width=X, height=Y(up), depth=Z
    return buildRoundedBox(params.width ?? 20, params.height ?? 20, params.depth ?? 20, r)
  }
...
function getMirrorMatrix(plane: string): number[] {
  switch (plane) {
    case 'YZ': return [-1, 0, 0,  0, 1, 0,  0, 0, 1,  0, 0, 0]
    case 'XZ': return [ 1, 0, 0,  0,-1, 0,  0, 0, 1,  0, 0, 0]
    case 'XY': return [ 1, 0, 0,  0, 1, 0,  0, 0,-1,  0, 0, 0]
    default:   return [ 1, 0, 0,  0, 1, 0,  0, 0, 1,  0, 0, 0]
  }
}
  ...
  try {
    switch (msg.type) {
      ...
      case 'buildShape': {
        ...
        const transform = msg.transform as { x:number;y:number;z:number;rotX:number;rotY:number;rotZ:number }
        ...
        if (shapeType === 'import_mesh') {
          ...
          m = new wasm.Manifold({ numProp: 3, vertProperties: verts, triVerts: tris })
        } else {
      ...
      case 'buildImportedMesh': {
        ...
        try {
          const m = new wasm.Manifold({ numProp: 3, vertProperties: verts, triVerts: tris })
          cache.set(msg.objId as string, m)
          const { vertices, indices, tris: numTris } = extractMesh(m)
          ;(self as unknown as Worker).postMessage(
            { reqId: msg.reqId, type: 'mesh', objId: msg.objId, vertices, indices, tris: numTris, ms: performance.now() - t0 },
            [vertices.buffer, indices.buffer],
          )
        } catch (me) {
          ...
          ;(self as unknown as Worker).postMessage(
            { reqId: msg.reqId, type: 'mesh', objId: msg.objId, vertices, indices, tris: tris.length / 3, ms: performance.now() - t0, nonManifold: true },
            [vertices.buffer, indices.buffer],
          )
        }
        break
      ...
      case 'csgBoolean': {
        ...
        switch (msg.op) {
          ...
          default:         result = a.intersect(b); break
        }
      ...
      case 'rebuildScene': {
        ...
        const currentTransforms: Map<string, { x:number;y:number;z:number;rotX:number;rotY:number;rotZ:number }> = new Map()
        ...
        for (const op of ops) {
          if (op.type === 'add_shape') {
            const t = op.transform as { x:number;y:number;z:number;rotX:number;rotY:number;rotZ:number }
            ...
            const verts = new Float32Array(op.vertices as number[])
            const tris  = new Uint32Array(op.indices as number[])
            try {
              const m = new wasm.Manifold({ numProp: 3, vertProperties: verts, triVerts: tris })
              cache.set(op.id as string, m)
            ...
            for (const id of op.ids as string[]) {
              ...
              if (t) {
                ...
                if (info) {
                  ...
                  let m = buildPrimitiveWithFillet(info.shapeType, info.params, info.filletRadius)
                  m = applyTransform(m, nt)
                  cache.set(id, m)
                } else {
                  // CSG результат — применяем трансляцию
                  const cm = cache.get(id)
                  if (cm) cache.set(id, cm.transform([1,0,0, 0,1,0, 0,0,1, d.x, d.y, d.z]))
                }
              } else {
                const cm = cache.get(id)
                if (cm) cache.set(id, cm.transform([1,0,0, 0,1,0, 0,0,1, d.x, d.y, d.z]))
              ...
              for (const [id, delta] of Object.entries(deltas)) {
                ...
                if (t) {
                  ...
                  if (info) {
                    ...
                    const cm = cache.get(id)
                    if (cm) cache.set(id, cm.transform([1,0,0, 0,1,0, 0,0,1, dx, dy, dz]))
                  }
                } else {
                  const cm = cache.get(id)
                  if (cm) cache.set(id, cm.transform([1,0,0, 0,1,0, 0,0,1, dx, dy, dz]))
            ...
            if (a && b) {
              ...
              for (const id of ids) { cache.delete(id); shapeInfos.delete(id); currentTransforms.delete(id) }
              const rid = op.resultId as string | undefined
              if (rid) cache.set(rid, result)
            }
      ...
      default:
        self.postMessage({ reqId: msg.reqId, type: 'error', message: `Unknown: ${msg.type}` })
    }
  } catch (err) {
