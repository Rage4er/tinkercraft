...
| **Y.2 Экономика V12** | 🔄 Текущий этап | P0 |
| **Y.3 Релиз в Яндекс.Игры** | 🔲 После Y.2 | P0 |
...
| Этап | Статус |
|------|--------|
| **1. CAD v1.0.0** | **✅ 19.08.2026** — Фазы 0–7.6, релиз, тег |
...
| | Блок A: Архитектура | ✅ 7.5.1–7.5.5 |
| | Блок B: Интерфейс | ✅ Все задачи |
| Фаза 7.6 — Mirror Pipeline Performance | ✅ Завершена |
| **2a. Y.1 — Фундамент SDK** | **✅ 21.08.2026** — IPlatform, SDK, GameplayAPI, COEP |
| **2b. Y.2 — Экономика V12** | **🔄 Текущий этап** — economy-store, quest-store, UI, бейджи, i18n |
| **2c. Y.3 — Релиз в Яндекс** | **🔲 Не начата** — модерация, метрики |
| Фаза 8 — Параметрическая история операций | 🔲 Не начата (после 2c) |
...
## 🗺️ Дорожная карта (обновлено 2026-08-23)

### Этап 1 — CAD v1.0.0 ✅ (19.08.2026)

Фазы 0–7.6, релиз, тег `v1.0.0`, деплой на GitHub Pages / Vercel.
~291 проблем код-ревью закрыты, 0 активных, 236 тестов проходят.
...
| **SDK инициализация** | `src/platform/sdk.ts`, `App.tsx` | Централизованная `initSdk()` + `initPlatform()` |
| **SDK в HTML** | `index.html` | `<script src="/sdk.js">` (требование платформы, п. 1.1) |
| **Gameplay API** | `src/App.tsx` | stop/start при открытии/закрытии модалок |
| **COEP-разделение** | `vite.config.ts` | Разные заголовки для clean/yandex сборок |
| **Сборка** | `vite.config.ts`, `package.json` | `dev:yandex`, `build:yandex` |
| **SDK-бейджи** | UI | Бейджи на платных кнопках (💎 / 📺) |

#### Y.2 Экономика V12 🔄 (текущий этап)

По [`ECONOMY.md`](ECONOMY.md):

| Компонент | Файлы | Описание |
|-----------|-------|----------|
| **Конфиг экономики** | `economy-config.ts` | Все цены/лимиты V12 в одном файле |
| **Economy store** | `economy-store.ts` | Токены, дневные лимиты, 24ч-пассы, подписки 700/2000, кэшбэк |
| **Quest store** | `economy-store.ts` | Квесты встроены в economy-store (3 квеста/день, пул 14) |
| **Триггеры** | `App.tsx` | subscribe к document-store: addShape, csg, setColor, mirror, align, export/import, sceneSize |
| **UI-панели** | `EconomyHUD.tsx`, `QuestPanel.tsx` | Токены, ежедневный бонус, квесты |
| **i18n EN/RU** | `locales/` | Переводы для всей экономики |

**Декомпозиция Y.2:**

| # | Задача | Оценка | Статус |
|---|--------|--------|--------|
| 1 | `economy-config.ts` — все цены/лимиты V12 в один файл | 0,5 ч | ✅ |
| 2 | `economy-store.ts` — токены, дневные лимиты, 24ч-пассы, подписки, кэшбэк (≤3/день, хэш модели) | 3 ч | ✅ |
| 3 | `economy-store.ts` — квесты (3/день, пул 14, midnight reset) | 2 ч | ✅ |
| 4 | Триггеры в subscribe (addShape, csg, setColor, mirror, align, export/import, sceneSize) | 2 ч | ✅ |
| 5 | UI: EconomyHUD + QuestPanel | 3 ч | ✅ |
| 6 | Бейджи на платные кнопки (экспорт/импорт/текст/палитра/баннер) | 1,5 ч | 🔲 Отложено |
| 7 | Модалка оплаты «токены / реклама» для разовых операций | 1,5 ч | 🔲 Отложено |
| 8 | i18n EN/RU для всей экономики | 1 ч | ✅ |
| 9 | Тест в песочнице (prod-proxy, app-id 572445) | 2 ч | 🔲 Отложено |
| - | **Исправление: удаление game-store.ts / GamePanel.tsx / UnlockModal.tsx / ToolBadge.tsx** | 1 ч | ✅ |
| | **Итого** | **≈ 17 ч** | |

> **Важно:** в V12 все фигуры бесплатны, замки/бейджи вешаются на платные **действия**: экспорт — «💎 50 / 📺 1», текст — «💎 75», и т.д. Старую схему «замок на торусе» не реализуем.
>
> **Исправлено 23.08:** Удалён дублирующий `game-store.ts` (V11: +5/+10 токенов), `GamePanel.tsx` (использовал старый стор), `UnlockModal.tsx`, `ToolBadge.tsx`. Бейджи на действиях (п.6) и модалка оплаты (п.7) отложены.

#### Y.3 Релиз в Яндекс.Игры 🔲

- Проверка политики баннера (отключение баннера может противоречить модерации)
- Ассеты: иконка TC 512×512, скриншоты, описание
- ZIP → загрузка в песочницу → модерация
- Мониторинг метрик 2–4 недели (Retention D1/D7, D1, реклама / DAU, конверсия в подписку)

### Этап 3 — Фаза 8 (CAD) 🔲

Старт только после Этапа 2. Параметрическая история операций:
- Timeline edit — UI для редактирования параметров операции
- Edit-модалки для примитивов, CSG, fillet, extrude, mirror
- Rebuild-on-edit — перестроение цепочки при изменении шага
- Исправление CSG-CSG-POSITION-DRIFT и MIRROR-CSG-CHILD-RS-LOSS
